...
namespace Oeuvre
{
	DX11VertexShader::DX11VertexShader(const char* filePath, D3D11_INPUT_ELEMENT_DESC* inputLayout, int inputLayoutSize)
	{
		if (!Create(filePath, inputLayout, inputLayoutSize))
			...
			OV_CORE_INFO("Compiled vertex shader from path \"{}\"", filePath);
	}
	...
	bool DX11VertexShader::Create(const char* path, D3D11_INPUT_ELEMENT_DESC* inputLayout, int inputLayoutSize)
	{
		...
		hr = device->CreateVertexShader(m_blobOut->GetBufferPointer(), m_blobOut->GetBufferSize(), 0, &m_vertexShader);
		if (FAILED(hr))
		{
			m_blobOut->Release();
			return false;
		...
		if (!CreateInputLayout(inputLayout, inputLayoutSize, m_blobOut))
		{
			m_blobOut->Release();
			return false;
		}
