...
namespace Oeuvre
{
	...
	void DX11RendererAPI::Shutdown()
	{
		...
		SAFE_RELEASE(m_pRasterizerCullFrontState);
	...
	void DX11RendererAPI::BeginFrame()
	{
		Clear();
	}

	void DX11RendererAPI::EndFrame(bool vSyncEnabled)
	{
		if (vSyncEnabled)
			GetSwapchain()->Present(1, 0);
		else
			GetSwapchain()->Present(0, DXGI_PRESENT_ALLOW_TEARING);
	}
