...
namespace Oeuvre
{
	...
	bool RenderTexture::Initialize(int width, int height, int mipMaps, DXGI_FORMAT textureFormat, DXGI_FORMAT RTVformat, DXGI_FORMAT DSVformat, DXGI_FORMAT SRVformat, bool cubeTexture, bool arrayTexture, int arraySize)
	{
	...
	void RenderTexture::SetRenderTarget(bool rtvToNull, bool depthToNull, bool bindUAV, ID3D11UnorderedAccessView** uav)
	{
		auto deviceContext = static_cast<DX11RendererAPI*>(RendererAPI::GetInstance().get())->GetDeviceContext();

		unsigned uavStartSlot = 1;
		if (rtvToNull)
		{
			if (depthToNull)
			{
				if (!bindUAV)
					...
					deviceContext->OMSetRenderTargetsAndUnorderedAccessViews(0, nullptr, nullptr, uavStartSlot, 1, uav, nullptr);
			}				
			else
			{
				if (!bindUAV)
					...
					deviceContext->OMSetRenderTargetsAndUnorderedAccessViews(0, nullptr, _pDepthStencilView, uavStartSlot, 1, uav, nullptr);
			}				
		}
		else
		{
			...
			{
				if (!bindUAV)
					deviceContext->OMSetRenderTargets(1, &_pRenderTargetView, nullptr);
				else
					deviceContext->OMSetRenderTargetsAndUnorderedAccessViews(1, &_pRenderTargetView, nullptr, uavStartSlot, 1, uav, nullptr);
			}				
			else
			{
				if (!bindUAV)
					...
					deviceContext->OMSetRenderTargetsAndUnorderedAccessViews(1, &_pRenderTargetView, _pDepthStencilView, uavStartSlot, 1, uav, nullptr);
			}				
		}
	}

	void RenderTexture::ClearRenderTarget(float red, float green, float blue, float alpha, float depth = 1.f) const
	{
		auto deviceContext = static_cast<DX11RendererAPI*>(RendererAPI::GetInstance().get())->GetDeviceContext();
