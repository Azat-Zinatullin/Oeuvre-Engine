...
namespace Oeuvre
{
	...
	bool DX11GBuffer::Initialize(int textureWidth, int textureHeight)
	{
        ...
        auto device = ((DX11RendererAPI*)RendererAPI::GetInstance().get())->GetDevice();
        // Create the render target textures.
        for (i = 0; i < GBUFFER_BUFFER_COUNT; i++)
        {
        ...
        depthBufferDesc.Format = DXGI_FORMAT_R32_TYPELESS;//DXGI_FORMAT_R24G8_TYPELESS;
    ...
    void DX11GBuffer::SetRenderTargets()
    {
        ...
        deviceContext->OMSetRenderTargets(GBUFFER_BUFFER_COUNT, m_RenderTargetViewArray, m_DepthStencilView);

        // Set the viewports.
        D3D11_VIEWPORT viewports[GBUFFER_BUFFER_COUNT];

        for (int i = 0; i < GBUFFER_BUFFER_COUNT; i++)
            viewports[i] = { 0.0f, 0.0f, (float)m_TextureWidth, (float)m_TextureHeight , D3D11_MIN_DEPTH, D3D11_MAX_DEPTH };

        deviceContext->RSSetViewports(GBUFFER_BUFFER_COUNT, viewports);
    }
    ...
    void DX11GBuffer::ClearRenderTargets(float red, float green, float blue, float alpha)
    {
        ...
        auto deviceContext = ((DX11RendererAPI*)RendererAPI::GetInstance().get())->GetDeviceContext();
        ...
        deviceContext->ClearDepthStencilView(m_DepthStencilView, D3D11_CLEAR_DEPTH, 1.0f, 0u);
    }
