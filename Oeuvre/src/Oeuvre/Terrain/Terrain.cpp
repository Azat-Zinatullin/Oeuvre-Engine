...
namespace Oeuvre
{
	...
	bool Terrain::Initialize(ID3D11Device* device, char* setupFilename)
	{
		...
		// Initialize the terrain height map with the data from the bitmap file or the raw file.
		if (std::string(setupFilename).find("setup_raw.txt") == std::string::npos)
	...
	bool Terrain::LoadBitmapHeightMap()
	{
		...
		{
			std::cout << "BIRMAP HEIGHT: " << bitmapInfoHeader.biHeight << ", BITMAP WIDTH: " << bitmapInfoHeader.biWidth << "\n";
			return false;
		}
		...
		if (count != imageSize)
		{
			std::cout << "ERROR::Can't read in the bitmap image data!\n";
			return false;
		}
	...
	bool Terrain::InitializeBuffers(ID3D11Device* device)
	{
		...
		{
			vertices[i].Pos = glm::vec3(m_terrainModel[i].x, m_terrainModel[i].y, m_terrainModel[i].z);
			vertices[i].Tex = glm::vec2(m_terrainModel[i].tu, m_terrainModel[i].tv);
			vertices[i].Normal = glm::vec3(m_terrainModel[i].nx, m_terrainModel[i].ny, m_terrainModel[i].nz);
			for (int k = 0; k < 4; k++)
			{
				vertices[i].BoneIDs[k] = 101;
				vertices[i].Weights[k] = 0.f;
			}
			indices[i] = i;
		}

		//std::cout << "INDICES SIZE: " << index << "\n";
